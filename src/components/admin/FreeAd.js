...
const FreeAd = () => {
    ...
    const initialState = {
        title: "", content: "", aboutFees: "", name: "", number: "", validation: false
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEditing) {
                await updateExistingFreeAd(isEditing, formData);
            } else {
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
                <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-2xl w-full max-w-2xl relative max-h-full overflow-y-auto">
                    ...
                    <form onSubmit={handleSubmit}>
                        <h2 className="text-2xl font-bold mb-6 text-gray-800 dark:text-white">{isEditing ? 'Edit' : 'Add'} Free Ad</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            ...
                            <input type="text" name="aboutFees" value={formData.aboutFees} onChange={handleChange} placeholder="About Fees" className="w-full p-3 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-orange-500 focus:border-orange-500" required />
                        ...
                        <div className="mb-4">
                            <textarea name="content" value={formData.content} onChange={handleChange} placeholder="Content" rows="4" className="w-full p-3 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-orange-500 focus:border-orange-500" required></textarea>
                        </div>
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-md">
            ...
            <div className="flex justify-between items-center mb-8">
                <h1 className="text-3xl font-bold text-gray-800 dark:text-white">Manage Free Ads</h1>
            ...
            {isLoading ? (
                <div className="text-center">Loading Ads...</div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {freeAds && freeAds.length > 0 ? freeAds.map((ad) => (
                        <div key={ad._id} className="bg-gray-50 dark:bg-gray-700 p-6 rounded-lg shadow-lg transition-transform hover:scale-105">
                            <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-2">{ad.title}</h3>
                            <p className="text-sm text-red-500 dark:text-red-400 font-bold mb-4">{ad.name}</p>
                            <div className="border-t border-gray-200 dark:border-gray-600 pt-4 flex justify-end gap-3">
                                <button onClick={() => handleEditClick(ad)} className="font-medium text-blue-500 hover:underline">Edit</button>
                                <button onClick={() => handleDelete(ad._id)} className="font-medium text-red-500 hover:underline">Delete</button>
                            </div>
                        </div>
                    )) : (
                        <p className="text-center col-span-full">No free ads found.</p>
                    )}
                </div>
            )}
        </div>
